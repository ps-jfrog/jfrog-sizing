...
| **Sizing bias** — Normal / Performance / Resilience | Normal sizes to the load baseline. Performance favors throughput (+1 tier when no license; skews licenses to primary when set). Resilience favors failover headroom (full passive mirror on warm A/P; skews licenses to secondary). Highlights when effective sizing differs from baseline |
| **Artifactory licenses purchased** — optional | Leave blank to size from load only (legacy behavior). When set, drives per-site Artifactory node counts and cascades tier/replicas to all components. Multi-site HA shows primary/secondary allocation (defaults to even split); hidden for single-replica or single-site |
...
The **load baseline tier** is `max(client-implied, RPM-implied)`, computed on growth-adjusted load. With no license entered, the **effective tier** follows that baseline (Normal bias), +1 tier (Performance), or full passive mirror (Resilience on warm A/P). When licenses are entered, per-site Artifactory node counts from the license split drive the effective tier and cascade to all components. The output highlights when effective sizing over- or under-allocates vs the load baseline.
