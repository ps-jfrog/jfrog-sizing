...
| **Artifactory licenses purchased** — optional | Leave blank to size from load only (legacy behavior). When set, drives per-site Artifactory node counts and cascades tier/replicas to all components. Split across sites follows topology (balanced/custom; warm reserves 1 for passive) |
